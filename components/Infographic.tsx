...
import { Download, Sparkles, Edit3, Maximize2, X, ZoomIn, ZoomOut, Headphones, Volume2, ClipboardCopy, Check, ThumbsUp, ThumbsDown } from 'lucide-react';
...
const Infographic: React.FC<InfographicProps> = ({ 
  ...
  const [generationRating, setGenerationRating] = useState<'up' | 'down' | null>(null);
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-6xl mx-auto animate-in fade-in zoom-in duration-700 mt-8">
      ...
      <div className="relative group w-full bg-slate-100 dark:bg-slate-900 rounded-2xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700/50">
        ...
        <div className="absolute top-2 right-2 md:top-6 md:right-6 flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-30">
          <button 
            onClick={() => setIsFullscreen(true)}
            className="bg-black/60 backdrop-blur-md text-white p-2 md:p-3 rounded-xl shadow-lg hover:bg-cyan-600 transition-colors border border-white/10 block"
      ...
      <div className="w-full max-w-3xl -mt-6 sm:-mt-8 relative z-40 px-4">
        <div className="bg-white/90 dark:bg-slate-800/80 backdrop-blur-xl p-3 sm:p-2 rounded-2xl shadow-2xl border border-slate-200 dark:border-white/10 flex flex-col sm:flex-row gap-2 items-center ring-1 ring-black/5 dark:ring-white/5">
            
            {/* Tools (Audio/Report/Rating) */}
            <div className="flex gap-1 pr-2 sm:border-r border-slate-200 dark:border-white/10 w-full sm:w-auto justify-center sm:justify-start">
                 <button 
                    onClick={handleCopy}
                    disabled={!hasInsights}
                    className={`p-2.5 rounded-xl transition-all flex items-center gap-2 ${showCopySuccess ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
                    title="Copy Strategic Report to Clipboard"
                 >
                    {showCopySuccess ? <Check className="w-4 h-4" /> : <ClipboardCopy className="w-4 h-4" />}
                 </button>

                 <button 
                    onClick={onPlayAudio}
                    disabled={isGeneratingAudio || !hasInsights}
                    className={`p-2.5 rounded-xl transition-all flex items-center gap-2 relative overflow-hidden ${isPlayingAudio ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'bg-slate-100 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 hover:text-indigo-600 dark:hover:text-indigo-400'}`}
                    title="Listen to Audio Brief (Podcast Mode)"
                 >
                    {isGeneratingAudio ? (
                        ...
                        <>
                             ...
                             <div className="flex items-end gap-[1px] h-3">
                                ...
                                <div className="w-1 bg-white animate-[bounce_0.4s_infinite] h-1"></div>
                             </div>
                        </>
                    ) : (
                 ...
                 {/* Generation Rating Controls */}
                 <div className="w-[1px] h-6 bg-slate-200 dark:bg-white/10 mx-1 self-center hidden sm:block"></div>
                 
                 <button 
                    onClick={() => setGenerationRating('up')}
                    className={`p-2.5 rounded-xl transition-all flex items-center gap-2 ${generationRating === 'up' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-700/50 text-slate-400 dark:text-slate-500 hover:text-emerald-500 dark:hover:text-emerald-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
                    title="Good generation"
                 >
                    <ThumbsUp className={`w-4 h-4 ${generationRating === 'up' ? 'fill-current' : ''}`} />
                 </button>

                 <button 
                    onClick={() => setGenerationRating('down')}
                    className={`p-2.5 rounded-xl transition-all flex items-center gap-2 ${generationRating === 'down' ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' : 'bg-slate-100 dark:bg-slate-700/50 text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
                    title="Bad generation"
                 >
                    <ThumbsDown className={`w-4 h-4 ${generationRating === 'down' ? 'fill-current' : ''}`} />
                 </button>
            </div>

            {/* Edit Input */}
            <div className="pl-2 text-cyan-600 dark:text-cyan-400 hidden sm:block">
